...
namespace BlogExample.MvcClient.Controllers
{
    ...
    public class AccountController : ServiceLocatorController
    {
        ...
        private TransactionScopeFactory tranScopeFactory = new TransactionScopeFactory();
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                try
                {
                    using (var scope = tranScopeFactory.Create())
                    {
                        result = CreateApplicationUser(model, out user);
        ...
        private IdentityResult CreateApplicationUser(RegisterViewModel model, out ApplicationUser user)
        {
            user = new ApplicationUser { UserName = model.Email, Email = model.Email };
            return UserManager.Create(user, model.Password);
        }

        private void CreateJournalUser(WebClientBL.Models.User user, IdentityResult result)
        {
            if (result.Succeeded)
            {
                ...
                using (var scope = tranScopeFactory.Create())
                {
                    try
                    {
                        ...
                        CreateJournalUser(
                            ...
                            result);
                        scope.Complete();
                        await SignInManager.SignInAsync(UserManager.FindByEmail(info.Email), isPersistent: false, rememberBrowser: false);
                        return RedirectToLocal(returnUrl);
                    }
                    catch
                    {
        ...
        private IdentityResult CreateExternalUser(ExternalLoginInfo info)
        {
